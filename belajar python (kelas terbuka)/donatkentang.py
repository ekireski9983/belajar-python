...
screen = donatkentang.display.set_mode((WIDTH, HEIGHT))

display_surface = donatkentang.display.set_mode((WIDTH, HEIGHT))
# display_surface = pygame.display.set_mode((0, 0), pygame.FULLSCREEN)
donatkentang.display.set_caption('Donut')
font = donatkentang.font.SysFont('Arial', 18, bold=True)
...
while run:
    ...
    donatkentang.display.update()

    hue += 0.005

    for event in donatkentang.event.get():
        if event.type == donatkentang.QUIT:
            run = False
        if event.type == donatkentang.KEYDOWN:
            if event.key == donatkentang.K_ESCAPE:
                run = False
